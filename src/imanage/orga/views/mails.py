...
from imanage.common.language import language
from imanage.common.mail import TolerantDict
from imanage.common.text.phrases import phrases
from imanage.common.ui import Button, LinkButton, delete_link, send_button
...
from imanage.mail.models import MailTemplate, QueuedMail, get_prefixed_subject
from imanage.mail.signals import request_pre_send
from imanage.orga.forms.mails import (
    DraftRemindersForm,
    MailDetailForm,
    MailTemplateForm,
...
from imanage.orga.tables.mail import MailTemplateTable, OutboxMailTable, SentMailTable
...
class MailDelete(PermissionRequired, ActionConfirmMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not mails:
            ...
            return redirect(self.request.event.orga_urls.outbox)
        for mail in mails:
            mail.log_action("imanage.mail.delete", person=self.request.user, orga=True)
            mail.delete()
...
class MailDetail(PermissionRequired, CreateOrUpdateView):
    ...
    extra_forms_signal = "imanage.orga.signals.mail_form"
    messages = {}
