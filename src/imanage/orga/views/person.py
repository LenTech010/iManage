...
from imanage.common.ui import Button
from imanage.common.views.generic import get_next_url
from imanage.common.views.helpers import is_form_bound
from imanage.orga.views.event import EventPermissionRequired
from imanage.person.forms import AuthTokenForm, LoginInfoForm, OrgaProfileForm
...
class UserSettings(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        if self.login_form.is_bound and self.login_form.is_valid():
            ...
            messages.success(request, phrases.base.saved)
            request.user.log_action("imanage.user.profile.update")
            ...
            if token:
                ...
                request.user.log_action(
                    "imanage.user.token.create", data=token.serialize()
                )
            ...
            request.user.log_action(
                "imanage.user.token.upgrade", data=token.serialize()
            )
            ...
            with scopes_disabled():
                ...
                if token:
                    ...
                    token.save()
                    request.user.log_action(
                        "imanage.user.token.revoke", data=token.serialize()
                    )
