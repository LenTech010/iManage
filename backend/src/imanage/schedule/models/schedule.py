...
from imanage.agenda.rules import can_view_schedule, is_agenda_visible, is_widget_visible
...
from imanage.orga.rules import can_view_speaker_names
from imanage.person.rules import is_reviewer
from imanage.schedule.models.slot import SlotType
from imanage.schedule.services import (
    freeze_schedule,
    get_cached_schedule_changes,
    unfreeze_schedule,
)
from imanage.submission.rules import is_wip, orga_can_change_submissions


class Schedule(ImanageModel):
    """The Schedule model contains all scheduled.

    :class:`~imanage.schedule.models.slot.TalkSlot` objects (visible or not)
    for a schedule release for an :class:`~imanage.event.models.event.Event`.
    ...
    def freeze(
        ...
        :param user: The :class:`~imanage.person.models.user.User` initiating
            the freeze.
    ...
    def scheduled_talks(self):
        """Returns all :class:`~imanage.schedule.models.slot.TalkSlot` objects
        that have been scheduled and are visible in the schedule (that is, have
        been confirmed at the time of release)."""
        from imanage.submission.models import SubmissionStates
    ...
    def slots(self):
        ...
        :class:`~imanage.submission.models.submission.Submission` objects with
        :class:`~imanage.schedule.models.slot.TalkSlot` objects in this
        schedule.
        """
    ...
    def get_talk_warnings(
        ...
        """
        from imanage.schedule.models import Availability, TalkSlot
    ...
    def generate_notifications(self, save=False):
        """A list of unsaved :class:`~imanage.mail.models.QueuedMail` objects
