...
from imanage.common.settings.config import build_config
from imanage.common.text.console import log_initial
...
DATA_DIR = Path(
    config.get(
        "filesystem",
        "data",
        fallback=os.environ.get("IMANAGE_DATA_DIR", BASE_DIR / "data"),
    )
)
...
PLUGINS = []
for entry_point in entry_points(group="imanage.plugin"):
...
CSRF_COOKIE_NAME = "imanage_csrftoken"
CSRF_TRUSTED_ORIGINS = [SITE_URL]
CSRF_COOKIE_SECURE = True
CSRF_COOKIE_HTTPONLY = False

SESSION_COOKIE_NAME = "imanage_session"
...
DEFAULT_EXCEPTION_REPORTER = "imanage.common.exceptions.ImanageExceptionReporter"
EMAIL_SUBJECT_PREFIX = "[imanage] "
MANAGERS = ADMINS = [email for email in emails if email]
if ADMINS:
    LOGGING["handlers"]["mail_admins"] = {
        "level": email_level,
        "class": "imanage.common.exceptions.ImanageAdminEmailHandler",
    }
...
FORMAT_MODULE_PATH = ["imanage.common.formats"]

LANGUAGE_CODE = config.get("locale", "language_code")
LANGUAGE_COOKIE_NAME = "imanage_language"
...
DEFAULT_EVENT_PRIMARY_COLOR = "#4B0082"
DEFAULT_EVENT_SECONDARY_COLOR = "#FFFF00"
...
DEFAULT_AUTHENTICATION_BACKENDS = [
    ...
    "imanage.common.auth.UserTokenAuthentication",
]
EXTRA_AUTH_BACKENDS = [
    backend
...
MIDDLEWARE = [
    ...
    "imanage.common.middleware.SessionMiddleware",  # Add session handling
    "django.contrib.auth.middleware.AuthenticationMiddleware",  # Uses sessions
    "imanage.common.middleware.auto_login.AutoLoginMiddleware",
    "csp.middleware.CSPMiddleware",  # Modifies/sets CSP headers
    "imanage.common.middleware.MultiDomainMiddleware",  # Check which host is used and if it is valid
    "imanage.common.middleware.EventPermissionMiddleware",  # Sets locales, request.event, available events, etc.
    "imanage.common.middleware.CsrfViewMiddleware",  # Protect against CSRF attacks before forms/data are processed
...
FORM_RENDERER = "imanage.common.forms.renderers.TabularFormRenderer"
...
TEMPLATES = [
    {
        ...
        "OPTIONS": {
            "context_processors": [
                ...
                "imanage.agenda.context_processors.is_html_export",
                "imanage.common.context_processors.add_events",
                "imanage.common.context_processors.locale_context",
                "imanage.common.context_processors.messages",
                "imanage.common.context_processors.system_information",
                "imanage.orga.context_processors.orga_events",
            ],
...
static_path = BASE_DIR / "imanage" / "static"
...
REST_FRAMEWORK = {
    ...
    "DEFAULT_AUTHENTICATION_CLASSES": ("imanage.common.auth.UserTokenAuthentication",),
    "DEFAULT_PERMISSION_CLASSES": ("imanage.api.permissions.ApiPermission",),
    ...
    "DEFAULT_PAGINATION_CLASS": "imanage.api.pagination.PageNumberPagination",
    "PAGE_SIZE": 50,
    "SEARCH_PARAM": "q",
    "ORDERING_PARAM": "o",
    "DATETIME_FORMAT": "iso-8601",
    "EXCEPTION_HANDLER": "imanage.api.exceptions.api_exception_handler",
}
if DEBUG:
...
SPECTACULAR_SETTINGS = {
    ...
    "VERSION": None,
    "TITLE": "imanage API",
    "SERVERS": [{"url": "https://imanage.com"}],
    "EXTERNAL_DOCS": {"url": "https://docs.imanage.org/api/"},
    ...
    "POSTPROCESSING_HOOKS": [
        ...
        "imanage.api.documentation.postprocess_schema",
    ],
}
REST_FLEX_FIELDS = {
    "WILDCARD_VALUES": [],
...
if "--no-imanage-information" in sys.argv:
    sys.argv.remove("--no-imanage-information")
elif not os.environ.get("IMANAGE_NO_INITIAL_LOG"):
    log_initial()
