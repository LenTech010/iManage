...
class ReviewScoreCategorySerializer(ImanageSerializer):
    class Meta:
...
@register_serializer(versions=CURRENT_VERSIONS)
class ReviewScoreSerializer(FlexFieldsSerializerMixin, ImanageSerializer):
    class Meta:
        model = ReviewScore
        fields = ("id", "category", "value", "label")
        expandable_fields = {
            "category": (
                "imanage.api.serializers.review.ReviewScoreCategorySerializer",
                {"read_only": True},
            ),
        }
...
@register_serializer(versions=CURRENT_VERSIONS)
class ReviewerSerializer(FlexFieldsSerializerMixin, ImanageSerializer):
    class Meta:
        model = User
...
@register_serializer(versions=CURRENT_VERSIONS)
class ReviewWriteSerializer(FlexFieldsSerializerMixin, ImanageSerializer):
    ...
    class Meta:
        ...
        expandable_fields = {
            ...
            "scores": (
                "imanage.api.serializers.review.ReviewScoreSerializer",
                {"read_only": True, "many": True},
            ),
        }
