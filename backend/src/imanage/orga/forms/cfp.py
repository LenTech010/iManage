...
from imanage.common.forms.renderers import InlineFormRenderer
...
from imanage.orga.forms.widgets import IconSelect
...
from imanage.submission.models.cfp import CfP
...
class CfPForm(ReadOnlyFlag, JsonSubfieldMixin, ImanageI18nModelForm):
...
class QuestionForm(ReadOnlyFlag, ImanageI18nModelForm):
...
class AnswerOptionForm(ReadOnlyFlag, ImanageI18nModelForm):
    class Meta:
        model = AnswerOption
        fields = ["answer"]


class SubmissionTypeForm(ReadOnlyFlag, ImanageI18nModelForm):
    def __init__(self, *args, event=None, **kwargs):
        self.event = event
        super().__init__(*args, **kwargs)
...
class TrackForm(ReadOnlyFlag, ImanageI18nModelForm):
    def __init__(self, *args, event=None, **kwargs):
        self.event = event
        super().__init__(*args, **kwargs)
        if self.instance.pk:
...
class CfPFieldConfigForm(ImanageI18nFormMixin, forms.Form):
    label = I18nFormField(
...
class StepHeaderForm(ImanageI18nFormMixin, forms.Form):
    title = I18nFormField(
        label=_("Step title"),
        required=False,
