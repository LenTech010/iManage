...
from imanage.common.exceptions import UserDeletionError
from imanage.common.image import create_thumbnail
from imanage.common.models import TIMEZONE_CHOICES
from imanage.common.models.mixins import FileCleanupMixin, GenerateCode, LogMixin
from imanage.common.text.path import path_with_hash
from imanage.common.urls import EventUrls, build_absolute_uri
from imanage.person.rules import is_administrator
...
class User(
    ...
    """The imanage user model.

    Users describe all kinds of persons who interact with imanage: Organisers, reviewers, submitters, speakers.
    ...
    :param groups: Django internals, not used in imanage.
    :param user_permissions: Django internals, not used in imanage.
    """
    ...
    is_staff = models.BooleanField(
        default=False, help_text="A default Django flag. Not in use in imanage."
    )
    is_administrator = models.BooleanField(
        default=False,
        help_text="Should only be ``True`` for people with administrative access to the server imanage runs on.",
    )
    ...
    def save(self, *args, skip_gravatar_processing=False, **kwargs):
        ...
        if self.get_gravatar and update_gravatar and not skip_gravatar_processing:
            from imanage.person.tasks import gravatar_cache
    ...
    def event_profile(self, event):
        ...
        :class:`~imanage.person.models.profile.SpeakerProfile` for this user.

        :type event: :class:`imanage.event.models.event.Event`
        :retval: :class:`~imanage.person.models.profile.EventProfile`
        """
    ...
    def get_event_preferences(self, event):
        ...
        from imanage.person.models.preferences import UserEventPreferences
    ...
    def regenerate_token(self) -> Token:
        ...
        self.log_action(action="imanage.user.token.reset")
...
you have requested a new password for your imanage account.
...
the imanage robot""")
        ...
        self.log_action(
            action="imanage.user.password.reset", person=user, orga=bool(user)
        )
...
Your imanage account password was just changed.
...
the imanage team""")
        ...
        with override(self.locale):
            QueuedMail(
                subject=_("[imanage] Password changed"),
                text=str(mail_text).format(**context),
                locale=self.locale,
                to=self.email,
            ).send()

        self.log_action(action="imanage.user.password.changed", person=self)
...
This is a confirmation that the email address for your imanage account has been changed from {old_email} to {new_email}.
...
the imanage team""")
        ...
        with override(self.locale):
            QueuedMail(
                subject=_("[imanage] Email address changed"),
                text=str(mail_text).format(**context),
                to=old_email,
                locale=self.locale,
            ).send()
        ...
        self.log_action(
            action="imanage.user.email.update",
            person=self,
            orga=False,
