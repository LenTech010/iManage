...
class ImanageExceptionReporter(ExceptionReporter):
    ...
    def get_traceback_text(self):  # pragma: no cover
        ...
        intro = f"""
You are receiving this email because an error occurred in your imanage installation at {settings.SITE_URL}.
You can find the technical details below – if you find that the problem was not due to a configuration error,
please report this issue at

    https://github.com/imanage/imanage/issues/new/choose
...
class ImanageCeleryExceptionReporter(ImanageExceptionReporter):
...
class ImanageAdminEmailHandler(AdminEmailHandler):
    reporter_class = ImanageExceptionReporter
