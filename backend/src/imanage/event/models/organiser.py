...
from imanage.event.models.event import FULL_SLUG_REGEX
...
class Organiser(ImanageModel):
    ...
    def shred(self, person=None):
        ...
        ActivityLog.objects.create(
            person=person,
            action_type="imanage.organiser.delete",
            content_object=self,
            is_orga_action=True,
            data=json.dumps(
                {
...
class Team(ImanageModel):
    """A team is a group of people working for the same organiser.

    Team members (of type :class:`~imanage.person.models.user.User`) share
    permissions for one or several events of
    :class:`~imanage.event.models.organiser.Organiser`.  People can be in
