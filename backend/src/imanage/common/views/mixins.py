...
from imanage.common.forms import SearchForm
...
class PermissionRequired(PermissionRequiredMixin):
    ...
    def has_permission(self):
        ...
        if not result:
            ...
            if request and hasattr(request, "event"):
                key = f"imanage_event_access_{request.event.pk}"
    ...
    def handle_no_permission(self):
        ...
        if request and request.user.is_anonymous:
            ...
            return redirect(reverse("orga:login") + f"?next={quote(request.path)}" + params)
        raise Http404()
...
class PaginationMixin:
    ...
    def get_context_data(self, **kwargs):
        from imanage.common.views.generic import CRUDView
