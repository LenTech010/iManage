...
class UpdateSettingsForm(GlobalSettingsForm):
    update_check_enabled = forms.BooleanField(
        ...
        help_text=_(
            "During the update check, imanage will report an anonymous, unique installation ID, "
            "the current version of imanage and your installed plugins and the number of active and "
            "inactive events in your installation to servers operated by the imanage developers. We "
    ...
    update_check_email = forms.EmailField(
        ...
        help_text=_(
            ...
            "address will not be transmitted to imanage.com, the emails will be sent by your server "
            "locally."
        ),
    )
