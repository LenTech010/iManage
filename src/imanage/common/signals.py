...
def minimum_interval(
    ...
    def decorate(func):
        ...
        def wrapper(*args, **kwargs):
            key_running = f"imanage_periodic_{func.__module__}.{func.__name__}_running"
            key_result = f"imanage_periodic_{func.__module__}.{func.__name__}_result"
...
"""
This is a regular django signal (no imanage event signal) that we send out every
...
subclass of imanage.common.exporter.BaseExporter
...
``imanage.common.signals.activitylog_display`` will be sent out with an ``activitylog``
argument.
...
to a human user, ``imanage.common.signals.activitylog_object_link`` will be sent out
...
"""
Responses to the ``imanage.common.signals.auth_html`` signal will be displayed as
...
the imanage settings file, and return a list of the registered locales as response
to this plugin signal. Every entry should be a tuple of two strings, the first being
the locale code, the second being the display name of the locale. (Though imanage will
