...
from imanage.common.language import LANGUAGE_NAMES
from imanage.common.models import TIMEZONE_CHOICES
from imanage.common.models.fields import DateField
from imanage.common.models.mixins import OrderedModel, ImanageModel
from imanage.common.models.settings import hierarkey
...
from imanage.event.rules import (
...
class Event(ImanageModel):
    ...
    :param is_public: Is this event public yet? Should only be set via the
        ``imanage.orga.views.EventLive`` view or in another way that processes
        the ``imanage.orga.signals.activate_event`` signal.
    ...
    primary_color = models.CharField(
        ...
        help_text=_(
            "Provide a hex value like #00ff00 if you want to style imanage in your event’s colour scheme."
        ),
    )
    ...
    def available_content_locales(self) -> list:
        # Content locales can be anything imanage knows as a language, merged with
    ...
    def _get_default_submission_type(self):
        from imanage.submission.models import SubmissionType
    ...
    def get_mail_template(self, role):
        from imanage.mail.default_templates import get_default_template
    ...
    def build_initial_data(self):
        ...
        from imanage.submission.models import CfP
        ...
        if not self.score_categories.all().exists():
            from imanage.submission.models import ReviewScore, ReviewScoreCategory
    ...
    def copy_data_from(self, other_event, skip_attributes=None):
        from imanage.orga.signals import event_copy_data
    ...
    def pending_mails(self) -> int:
        ...
        :class:`~imanage.mail.models.QueuedMail` objects.
        """
    ...
    def wip_schedule(self):
        ...
        :retval: :class:`~imanage.schedule.models.schedule.Schedule`
        """
        try:
    ...
    def get_mail_backend(self, force_custom: bool = False) -> BaseEmailBackend:
        from imanage.common.mail import CustomSMTPBackend
    ...
    @cached_property
    def teams(self):
        """Returns all :class:`~imanage.event.models.organiser.Team` objects
    ...
    def talks(self):
        ...
        :class:`~imanage.submission.models.submission.Submission` object in the
        current released schedule.
        """
        from imanage.submission.models.submission import Submission
    ...
    def speakers(self):
        ...
        :class:`~imanage.person.models.user.User`) visible in the current
        released schedule.
        """
        from imanage.person.models import User
    ...
    @cached_property
    def submitters(self):
        """Returns a queryset of all :class:`~imanage.person.models.user.User`
    ...
    def release_schedule(
        ...
        """Releases a new :class:`~imanage.schedule.models.schedule.Schedule`
        by finalising the current WIP schedule.

        :param name: The new version name
        :param user: The :class:`~imanage.person.models.user.User` executing the release
        :param notify_speakers: Generate emails for all speakers with changed slots.
        :param comment: Public comment for the release
        :type user: :class:`~imanage.person.models.user.User`
        """
...
The IManage Team
    ...
    def has_unreleased_schedule_changes(self) -> bool:
        ...
        """
        from imanage.schedule.services import has_unreleased_schedule_changes
...
class EventExtraLink(OrderedModel, ImanageModel):
