...
env_prefix = "IMANAGE_INIT_ORGANISER_"
...
class Command(BaseCommand):  # pragma: no cover
    help = "Initializes your imanage instance. Only to be used once."
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS(
                "\nWelcome to imanage! This is my initialization command, please use it only once."
            )
        )
                ...
                if not environ.get(env_var):
                    self.stdout.write(
                        self.style.ERROR(
                            ...
                            "Please refer to the documentation: https://docs.imanage.org/administrator/commands/#init"
                        )
                    )
                    sys.exit(-1)

        self.stdout.write(
            "\nLet's get you a user with the right to create new events and access every event on this imanage instance."
        )
