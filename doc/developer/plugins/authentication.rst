...
An authentication plugin allows you to add a new way to allow users to log in to your imanage
...
`imanage-social-auth <https://github.com/adamskrz/imanage-social-auth>`_, which also provides
...
The Imanage auth plugin system is based around Django’s authentication backends, with a couple
of signals to integrate with the Imanage interface.
...
To add your the backend to Imanage, it needs to be added `authentication` section of the
`imanage.cfg` file, in addition to the standard plugin registration e.g.:
...
.. code-block:: ini
   ...
   my_auth_plugin = "my_auth_plugin:ImanagePluginMeta"

The main signal used for authentication plugins (that require visiting a custom view) is the
`imanage.common.signals.auth_html` signal. This adds additional HTML to the login page, for
...
parameter, which is the path the user should be redirected to after logging in.::
   ...
   from imanage.common.signals import auth_html
...
Most authentication plugins will store data against a user in the Imanage database (e.g.
...
remove any associated data for the `user` parameter.::
   ...
   from imanage.common.signals import delete_user
