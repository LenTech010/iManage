...
from imanage.agenda.rules import (
    event_uses_feedback,
...
from imanage.common.models.choices import Choices
from imanage.common.models.fields import MarkdownField
from imanage.common.models.mixins import GenerateCode, ImanageModel
from imanage.common.text.path import path_with_hash
from imanage.common.text.phrases import phrases
from imanage.common.text.serialize import serialize_duration
from imanage.common.urls import EventUrls
from imanage.mail.models import get_prefixed_subject
...
class Submission(GenerateCode, ImanageModel):
    """Submissions are, next to :class:`~imanage.event.models.event.Event`, the
    central model in imanage.

    A submission, which belongs to exactly one event, can have multiple
    speakers and a lot of other related data, such as a
    :class:`~imanage.submission.models.type.SubmissionType`, a
    :class:`~imanage.submission.models.track.Track`, multiple
    :class:`~imanage.submission.models.question.Answer` objects, and so on.
    ...
    title = models.CharField(max_length=200, verbose_name=_("Paper title"))
    ...
    def public_answers(self):
        from imanage.submission.models.question import QuestionTarget
    ...
    def get_duration(self) -> int:
        ...
        Falls back to the
        :class:`~imanage.submission.models.type.SubmissionType`'s default
    ...
    def update_duration(self):
        ...
        :class:`~imanage.schedule.models.slot.TalkSlot`.
    ...
    def update_talk_slots(self):
        ...
        :class:`~imanage.schedule.models.slot.TalkSlot` objects exists.
    ...
    def send_initial_mails(self, person):
        from imanage.mail.models import MailTemplateRoles
    ...
    def make_submitted(
        ...
        if previous != SubmissionStates.DRAFT:
            self.log_action(
                "imanage.submission.make_submitted",
                person=person,
                orga=orga,
    ...
    def reject(
        ...
        :class:`~imanage.mail.models.QueuedMail`.
        """
        previous = self.state
        self._set_state(SubmissionStates.REJECTED, force, person=person)
        self.log_action(
            "imanage.submission.reject",
            person=person,
            orga=True,
            data={"previous": previous, "from_pending": from_pending},
        )
    ...
    @cached_property
    def slot(self):
        """The first scheduled :class:`~imanage.schedule.models.slot.TalkSlot`
        of this submission in the current.

        :class:`~imanage.schedule.models.schedule.Schedule`.
    ...
    def public_slots(self):
        """All publicly visible :class:`~imanage.schedule.models.slot.TalkSlot`
    ...
    def speaker_profiles(self):
        from imanage.person.models.profile import SpeakerProfile
    ...
    def availabilities(self):
        ...
        :class:`~imanage.schedule.models.availability.Availability` objects of
        all speakers of this submission.
        """
        from imanage.schedule.models.availability import Availability
    ...
    def add_speaker(self, email, name=None, locale=None, user=None):
        from imanage.common.urls import build_absolute_uri
        from imanage.mail.models import MailTemplateRoles
        from imanage.person.models import SpeakerProfile, User
        from imanage.person.services import create_user
        ...
        self.log_action(
            "imanage.submission.speakers.add",
            person=user,
            orga=True,
    ...
    def remove_speaker(self, speaker, orga=True, user=None):
        if self.speakers.filter(code=speaker.code).exists():
            ...
            self.log_action(
                "imanage.submission.speakers.remove",
...
class SubmissionFavourite(ImanageModel):
    user = models.ForeignKey(
        to="person.User",
        on_delete=models.CASCADE,
...
class SubmissionInvitation(ImanageModel):
    ...
    def retract(self, person=None, orga=False):
        ...
        submission.log_action(
            "imanage.submission.invitation.retract",
            person=person,
            orga=orga,
            data={"email": email},
        )
